        ...
        with nc.Dataset(cfg.tnofile) as tno:
            ...
            for cat in cfg.output_cat:
                ...
                selection_cat = np.array(
                    [tno["emission_category_index"][:] == cfg.tno_cat.index(cat)+1])

                """mask corresponding to the given category for area/point"""
                selection_cat_area  = np.array([selection_cat.any(0),selection_area]).all(0)
                selection_cat_point = np.array([selection_cat.any(0),selection_point]).all(0)
