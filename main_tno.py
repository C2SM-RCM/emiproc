        ...
        for f in list_input_files:
            ...
            with nc.Dataset(f) as tno:
                interpolation = get_interpolation(
                    cfg, tno, filename="mapping_tno.npy"
                )
                ...
                for snap in cfg.snap:
                    ...
                    selection_snap_area = np.array(
                        [selection_snap.any(0), selection_area]
                    ).all(0)
                    selection_snap_point = np.array(
                        [selection_snap.any(0), selection_point]
                    ).all(0)
                    ...
                    for s in species_list:
                        ...
                        for (i, source) in enumerate(var):
                            if selection_snap_area[i]:
                                ...
                                for (x, y, r) in interpolation[
                                    lon_ind, lat_ind
                                ]:
                                    out_var_area[y, x] += var[i] * r
                            if selection_snap_point[i]:
                                (indx, indy) = interpolate_tno_to_cosmo_point(
                                    i, tno, cfg
                                )
                                if (
                                    indx >= 0
