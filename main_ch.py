...
def get_ch_emi(filename):
    ...
    emi_new = np.empty(
        (np.shape(emi)[0], np.shape(emi)[1])
    )  # create the same dimention as emi
...
def main(cfg_path):
    ...
    print(
        'Only use data inside "CH" according to country_mask '
            ...
            for var in cfg.species:
                if cfg.origin == "meteotest":
                    constfile = os.path.join(
                        cfg.input_path,
                        "".join(["e", cat.lower(), "15_", var.lower(), "*"]),
                    )
                    ...
                    constfile = os.path.join(
                        cfg.input_path, "".join(["ch4_", cat.lower(), ".txt"])
                    )
                    out_var_name = var + "_" + cfg.mapping[cat]
                else:
