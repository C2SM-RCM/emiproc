...
def doit(i, interp_tot, cosmo_area):
    ...
    for s, sname in zip(["gpp_", "ra_"], ["CO2_GPP_F", "CO2_RA_F"]):
        for h in range(24):
            ...
            output_path = (
                "/scratch/snx3000/haussaij/VPRM/output/" + filename_out
            )
            if os.path.exists(output_path):
                print("exists:", output_path)
                continue
            data_path = (
                "/scratch/snx3000/haussaij/VPRM/input/"
                + inidate.strftime("%Y%m%d")
                + "/"
                + s
                + "_"
                + time_str
            ...
            for lon in range(data.shape[1]):
                for lat in range(data.shape[0]):
                    for (x, y, r) in interp_tot[lat, lon]:
                        ...
                        out_var_area[y, x] += (
                            data[lat, lon] * r * cfg.tno_dx * cfg.tno_dy
                        )  # now out_var_area is in umol.cell-1.s-1
...
def get_interp_tot(cfg):
    for n, i in enumerate(
        [6, 2, 4, 1, 3, 5]
    ):  # [1,3,5,6,2,4]): # Weird order, following the README of Julia
        input_path = cfg.vprm_path % str(i)
        with Dataset(input_path) as inf:
            interpolation = get_interpolation(
                cfg,
                inf,
                filename="EU" + str(i) + "_mapping.npy",
                inv_name="vprm",
            )
...
def main(cfg_path):
    ...
    with Pool(36) as pool:
        pool.starmap(
            doit, [(i, interp_tot, cosmo_area) for i in range(10)]
        )  # range(1,10)])
